...
public class TiempoCiclistaComparador implements Comparator<Ciclista>
{
    ...
    {
        if(Math.abs(c1.getResultados().get(c1.getEtapaActual()).getTiempo() - c2.getResultados().get(c2.getEtapaActual()).getTiempo()) <= 0.001)
        {
            return Collections.reverseOrder(new NombreCiclistaComparador()).compare(c1,c2);
        }
        else if(c1.getResultados().get(c1.getEtapaActual()).getTiempo() < c2.getResultados().get(c2.getEtapaActual()).getTiempo())
        {
            return 1;
        }
        else
